...
describe('MasterEngine', () => {
  ...
  describe('_scoreSignal()', () => {
    ...
    it('weak setup (only macro) scores below 75 (XAU/USD intraday threshold)', () => {
      const { total } = engine._scoreSignal({
        action:      'BUY',
  ...
  describe('analyze() — integration (mocked API)', () => {
    ...
    it('does not fire XAU/USD signal when ATR5m is below minimum (stale data guard)', async () => {
      // Candles with tiny range = stale data scenario (ATR << 1.5 for XAU)
      const staleCandles = Array.from({ length: 100 }, (_, i) => ({
        open: 5278, high: 5278.05, low: 5277.95, close: 5278 + i * 0.001, time: new Date().toISOString()
      }));
      const stale1m = Array.from({ length: 50 }, (_, i) => ({
        open: 5278, high: 5278.02, low: 5277.98, close: 5278 + i * 0.0005, time: new Date().toISOString()
      }));
      // Use bullish 1H so macro passes, but stale 5m so ATR is tiny
      mockFetchCandles(engine,
        makeBullishCandles(100, 5200, 5), // 1H bullish — passes macro
        makeBullishCandles(100, 5200, 3), // 15m bullish
        staleCandles,                      // 5m stale — ATR ~0.05, below 1.5 min
        stale1m                            // 1m stale
      );
      const signals = await engine.analyze('XAU/USD', 'NEUTRAL', 15);
      // Should be null OR empty array — ATR guard blocks stale signals
